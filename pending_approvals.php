    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_invoice'])) {
        ...
        elseif (!isset($_POST['confirm_approval'])) {
            ...
            // Modificado: Usar siempre permisos de admin para aprobar facturas
            $result = approveInvoice($invoice_id, $user_id, 'admin', $comments);
    ...
    // Obtener todas las facturas pendientes como si fuera admin para todos los usuarios
    $pending_invoices = getPendingInvoicesForRole('admin');

    // Función auxiliar para obtener facturas pendientes para un rol específico
    function getPendingInvoicesForRole($role) {
        $conn = getDbConnection();

        // Obtener todas las facturas que no están completadas ni rechazadas
        $sql = "SELECT * FROM invoices WHERE status NOT IN ('completado', 'rechazado') ORDER BY date DESC";
        ...
        foreach ($invoices as $invoice) {
            // Obtener las aprobaciones de la factura
            $approvals = getInvoiceApprovals($invoice['id']);

            $hasApproval = false;

            // Verificar si el rol ya aprobó la factura
            foreach ($approvals as $approval) {
                if ($approval['action'] == 'approve' && $approval['user_role'] == $role) {
                    $hasApproval = true;
                    break;
                }
            }

            // Si el rol ya aprobó la factura, no agregarla a la lista para ese rol
            if (!$hasApproval) {
                // Verificar que el rol no haya rechazado la factura
                $hasRejected = false;
                foreach ($approvals as $approval) {
                    if ($approval['action'] == 'reject' && $approval['user_role'] == $role) {
                        $hasRejected = true;
                        break;
                    }
                }

                // Solo agregar la factura si no ha sido rechazada y el rol aún no ha aprobado
                if (!$hasRejected) {
                    $filtered[] = $invoice;
                }
            }
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="btn-group">
                                                            ...
                                                            <?php
                                                            // Modificado: permitir aprobación para cualquier rol
                                                            ...
                                                            <!-- Modificado: permitir rechazo para cualquier rol -->
